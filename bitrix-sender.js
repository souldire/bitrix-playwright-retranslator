...
class BitrixSender {
  ...
  // капчу вручную. Когда вход подтверждён (несколько секунд подряд открыто
  // поле входа → раздел портала), сервис сам сохраняет куки и сам закрывает
  // окно — закрывать руками не нужно. После этого поднимается обычный браузер
  // и по auth-state.json проверяется, что сессия жива.
  async _doInteractiveLogin(reason) {
    ...
    console.warn(`После входа окно закроется само (или закрой раньше). Жду до ${waitMinutes} мин.`);
    ...
    while (Date.now() - start < INTERACTIVE_LOGIN_TIMEOUT_MS) {
      if (!this.browser.isConnected()) break; // пользователь закрыл окно сам

      if (this._isPortalUrl(this.page.url())) {
        portalTicks += 1;
      } else {
        portalTicks = 0;
      }

      if (!hinted && portalTicks > 0) {
        hinted = true;
        console.log('Похоже, вход выполнен — закрываю окно через несколько секунд.');
      }

      // Вход подтверждён несколько секунд подряд (устойчиво открыт раздел
      // портала, а не страница входа и не мигающая перезагрузка) — заканчиваем
      // сами: сохраняем сессию и закрываем окно, руки пользователя не нужны.
      if (portalTicks >= 5) {
        await this._saveStorageState();
        break;
      }
