...
2. Открывается видимый (не headless) браузер с уже заполненными логином и паролем. Решите капчу, если она появилась, и нажмите «Войти».
3. Пока окно открыто, сервис раз в пару секунд сохраняет куки в `auth-state.json`. Сервис сам не решает, когда вход выполнен: **когда войдёте — закройте окно браузера сами**. После закрытия сервис поднимет обычный браузер, проверит по сохранённым кукам, что сессия жива, и продолжит отправку.
4. Если за `INTERACTIVE_LOGIN_TIMEOUT_MS` (по умолчанию 5 минут) окно так и не закрыли — сервис закроет его сам, сообщение вернётся в очередь и будет повторено через `AUTH_RETRY_DELAY_MS` (по умолчанию 10 минут). Если окно закрыли без входа — повтор тоже произойдёт после той же паузы.
