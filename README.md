...
3. После успешного входа сессия автоматически сохраняется в `auth-state.json`, и дальше сервис снова работает по кукам, без пароля и капчи.
4. Если за `INTERACTIVE_LOGIN_TIMEOUT_MS` (по умолчанию 5 минут) вход не выполнен или окно закрыли — сообщение вернётся в очередь и будет повторено через `AUTH_RETRY_DELAY_MS` (по умолчанию 10 минут).
